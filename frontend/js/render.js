...
function mediaEl(className, image, icon, alt) {
  if (image) {
    return el("div", { class: className }, [
      el("img", { src: "../images/" + image, alt, loading: "lazy" }),
    ]);
  }
  return el("div", { class: className, text: icon });
}

function packageCard(pkg, { compact } = {}) {
  const media = mediaEl("card-media", pkg.image, pkg.icon, pkg.title);
...
function destinationCard(dest) {
  const media = mediaEl("card-media", dest.image, dest.icon, dest.title);
...
function fullPackageCard(pkg) {
  const media = mediaEl("package-media", pkg.image, pkg.icon, pkg.title);
...
const PACKAGE_CATEGORY_ORDER = ["Bhutan Tours", "Bhutan Treks", "Festival Tours", "Special Interest"];
...
async function renderPackages(containerId) {
  ...
  const categories = PACKAGE_CATEGORY_ORDER.filter((cat) => packages.some((p) => p.category === cat));
  packages.forEach((p) => {
    if (!categories.includes(p.category)) categories.push(p.category || "More Packages");
  });

  categories.forEach((category) => {
    const inCategory = packages.filter((p) => (p.category || "More Packages") === category);
    if (!inCategory.length) return;
    container.appendChild(el("h2", { class: "package-category-heading", text: category }));
    inCategory.forEach((pkg) => container.appendChild(fullPackageCard(pkg)));
  });
}
